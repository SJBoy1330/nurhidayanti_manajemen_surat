...
function formposition(id) {
    ...
    let position = -1;
    id = id.replace(/#/g, ''); // Jangan pakai 'var' atau 'let' di sini
        ...
        success: function (data) {
            ...
            if (data.prop) {
                for (var a = 0; a < data.prop.length; a++) {
                    $(data.prop[a].parent).prop(data.prop[a].one,data.prop[a].two);
                }
            }
            if (data.datatable) {
            ...
            if (data.alert && cek_ketersediaan == true) {
                ...
                Swal.fire({
                    ...
                    if (data.element != null) {
                     ...
                     if (data.add_class != null) {
                        const row = data.add_class.length;
                        for (var i = 0; i < row; i++) {
                            $(data.add_class[i].id).addClass(data.add_class[i].class);
                        }
                    }
                    if (data.remove_class != null) {
                        const row = data.remove_class.length;
                        for (var i = 0; i < row; i++) {
                            $(data.remove_class[i].id).addClass(data.remove_class[i].class);
                        }
                    }
                    if (data.remove != null) {
                ...
                if (data.required) {
                    ...
                    for (var i = 0; i < array; i++) {
                        ...
                        lastreq = data.required[i][0];
                    }
                    if (lastreq) {
                        document.getElementById(lastreq).scrollIntoView();
                    }
...
function hapus_data(element, e, id, db = 'users', primary = '',file = '') {
    ...
    Swal.fire({
        ...
        if (t.isConfirmed) {
            $.ajax({
                url: BASE_URL + '/setting_function/hapus_data',
                method: 'POST',
                data: { 
                    id: id, db : db,
                    primary : primary,
                ...
                success: function (data) {
                    // console.log(data);
                    if (data.status == 200 || data.status == true) {
                        if ($(element).data('base_load') && $(element).data('base_pane')) {
                            let fullUrlWithoutQuery = window.location.origin + window.location.pathname;

                            var base_pane = $(element).data('base_pane');
                            var base_load = $(element).data('base_load');
                            $(base_pane).load(fullUrlWithoutQuery+' '+base_load);
                        }
                        if (data.alert) {
                            if (reload == 'big') {
                                if (data.alert) {
