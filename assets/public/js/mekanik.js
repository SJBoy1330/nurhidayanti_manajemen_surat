...
function modal_notification(id) {
    var menu = KTMenu.getInstance(document.getElementById('kt_menu_notifications'));
    if (menu) {
        menu.hide(); // ini akan menutup menu
    }
    $.ajax({
        url: BASE_URL + '/setting_function/get_notification',
        method: 'POST',
        data: { 
            id: id 
        },
        ...
        success: function (msg) {
            $('#list-'+id).addClass('bg-secondary');
            $('#list-'+id+' .symbol-label').removeClass('bg-light-primary');
            $('#list-'+id+' .symbol-label').addClass('bg-light-secondary');
            $('#list-'+id+' .symbol-label i').removeClass('text-white');
            $('#list-'+id+' .symbol-label i').addClass('text-muted');

            $('#display_data_notification').html(msg);
        }
    })
}
