...
document.addEventListener('DOMContentLoaded', function () {
    overtimeTable = initGlobalDatatable('#table_overtime', function () {
        return {
    ...
    document.querySelectorAll('.table-filter').forEach(el => {
        el.addEventListener('change', function () {
            if (overtimeTable) overtimeTable.ajax.reload();
        });
    });
});
...
function approved_submission(element,id,sts = 'N',text) {
    ...
    Swal.fire({
        ...
        if (t.isConfirmed) {
            $.ajax({
                url: BASE_URL + 'submission_function/approval/overtime/id_overtime',
                method: 'POST',
                data: { 
                    id: id,
                    status: sts
